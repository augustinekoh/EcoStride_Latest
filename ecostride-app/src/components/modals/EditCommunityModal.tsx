...
export function EditCommunityModal({ onClose, onUpdated, guild }: EditCommunityModalProps) {
  const [name, setName] = useState(guild.name || '');
  const [description, setDescription] = useState(guild.description || '');
  const [icon, setIcon] = useState(guild.icon || '🌍');
  const [nationality, setNationality] = useState(guild.nationality || 'Global');
  const [requireApproval, setRequireApproval] = useState(!!guild.require_approval);
  ...
  // if guild.icon is a URL, use it as initial preview
  const isUrl = guild.icon && (guild.icon.startsWith('http') || guild.icon.startsWith('/'));
  const [iconPreviewUrl, setIconPreviewUrl] = useState<string | null>(isUrl ? guild.icon : null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiClient(`/guilds/${guild.id}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          name,
          description,
          icon: icon || guild.icon, // If empty because image was selected, API will preserve old or expect upload
          nationality,
      ...
      if (data.success && iconFile) {
        ...
        try {
          const uploadRes = await apiClient(`/guilds/${guild.id}/icon`, {
            method: 'POST',
            body: formData
          });
    ...
    <div className="fixed inset-0 z-[100] flex flex-col justify-end pointer-events-auto">
      ...
      <div className="relative bg-[#faf9f6]/95 backdrop-blur-xl w-full h-[85vh] rounded-t-3xl shadow-2xl flex flex-col overflow-hidden border-t border-[#1d3539]/10 animate-in slide-in-from-bottom">
        ...
        <div className="px-6 py-4 flex justify-between items-center border-b border-[#1d3539]/10">
          <h2 className="text-xl font-black text-[#1d3539]">Edit Community</h2>
