...
export const ShareSignpostModal: React.FC<Props> = ({ signpostId, isOpen, onClose, onShared }) => {
  ...
  const handleShare = async (targetId: string, isFriend = false) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to share signpost');
    } finally {
      setSharing(false);
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[80vh] animate-in zoom-in-95">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
          <h2 className="font-bold text-lg text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <Send size={18} className="text-[#5496a2]" />
            Share Signpost
          </h2>
        ...
        <div className="p-4 overflow-y-auto">
          ...
          <div className="mb-6">
            ...
            {guildId ? (
              ...
              >
                <div className="w-12 h-12 bg-teal-100 dark:bg-teal-900/40 rounded-full flex items-center justify-center text-teal-600 dark:text-teal-400">
                  <Users size={24} />
                </div>
          ...
          <div>
            ...
            {loading ? (
              <div className="animate-pulse space-y-2">
                {[1, 2, 3].map(i => (
                  <div key={i} className="h-14 bg-slate-100 dark:bg-slate-800 rounded-2xl w-full"></div>
                ))}
              </div>
            ) : friends.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-500">Send private message</p>
                      </div>
                    </button>
                  );
                })}
